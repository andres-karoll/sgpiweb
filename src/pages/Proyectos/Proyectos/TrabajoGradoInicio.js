...
function TrabajoGradoInicio(){
    const [proyectos, setproyectos] = useState([]);
    const [tablaproyectos, settablaproyectos] = useState([]);
    const [busqueda, setbusqueda] = useState("");

    const peticionGet = async()=>{
        await axios.get("http://localhost:8080/gestionproyectosinvestigacion/TrabajoGradoInicio/"+localStorage.getItem("cedula"))
        .then(response=>{
      ...
      <div>
            <section className="content">
                ...
                <div class="alert alert-info alert-dismissible">
                  <h1><i class=" fas fa-cubes  nav-icon"></i>Todos los proyetos en estado inicial</h1>
                  </div>
                  </section>
            </div>     
...
export default TrabajoGradoInicio;
